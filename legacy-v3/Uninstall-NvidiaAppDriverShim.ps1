        ...
        if ([int]::TryParse($pidText.Trim(), [ref]$shimPid)) {
            ...
            if ($processInfo) {
                if (-not [string]::IsNullOrWhiteSpace($NodePath)) {
                    ...
                    $nodeMatches = (
                        [IO.Path]::GetFileName([string]$processInfo.ExecutablePath) -eq
                        'node.exe'
                    )
                }
            }
            if (
                $processInfo -and
                $nodeMatches -and
                ([string]$processInfo.CommandLine).Contains($proxyPath) -and
...
try {
    ...
    if ($state.status -eq 'uninstalled') {
        $nodePath = Get-ShimTaskNodePath -AllowMissing
        Stop-ShimTaskAndProcess -NodePath $nodePath
