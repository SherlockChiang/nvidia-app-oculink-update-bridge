...
if ([string]$state.verifiedLatestVersion -ne $expectedLatestVersion) {
    throw "Protected state did not record the verified $expectedLatestVersion recommendation."
}

$config = Get-Content -LiteralPath $configPath -Raw | ConvertFrom-Json
if ([int]$config.port -ne 80) {
    throw "The v3 helper must use implicit HTTP port 80; found '$($config.port)'."
}
$baseUrl =
    Get-LoopbackBaseUrl `
...
if ($baseUrl -notmatch '^http://127\.0\.0\.1/[a-f0-9]{32,128}/$') {
    throw 'The v3 loopback URL exposes an explicit port to NVIDIA App.'
}
...
if (
    ...
    throw 'The protected v3 helper configuration failed its SHA-256 check.'
}
if (
    [string]::IsNullOrWhiteSpace([string]$state.proxySha256) -or
    (Get-FileHash -LiteralPath $proxyPath -Algorithm SHA256).Hash -ne
        [string]$state.proxySha256
) {
    throw 'The installed v3 proxy failed its SHA-256 check.'
}
...
if (
    ...
    throw 'The live NVIDIA component profile changed after the v3 redirect was installed.'
}
...
if (
    ...
    throw 'The implicit-port loopback v3 helper health check failed.'
}
if (
...
if ($helperProcess.ProcessName -ne 'node') {
    throw 'The helper PID is not a Node.js process.'
}
...
if ($latestText -ne $expectedLatestVersion) {
    throw "NVIDIA metadata returned '$latestText', not expected release $expectedLatestVersion."
}
...
[pscustomobject]@{
    ...
    TaskAccount = $state.helperAccount
    UiRedirect = $state.uiRedirectStatus
    LocalizedServer = $localizedServer
